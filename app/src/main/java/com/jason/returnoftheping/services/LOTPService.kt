...
interface LOTPService {
    ...
    fun signIn(@Field("email") email: String,
               @Field("password") password: String): Observable<SignInRegisterResponse>

    @GET("v2/profile/{profileId}")
    fun getProfile(@Path("profileId") playerId: Int): Observable<Profile>
    ...
    @FormUrlEncoded
    fun confirmMatch(@Field("confirmed") confirmed: Boolean,
                     @Field("matchId") matchId: Int): Observable<MatchConfirmationResponse>

    @POST("v1/match")
    @FormUrlEncoded
    fun saveMatch(@Field("opponentId") opponentId: Int,
                  @Field("gamesWon") gamesWon: Int,
                  @Field("gamesLost") gamesLost: Int): Observable<MatchConfirmationResponse>

    @GET("v1/players")
    fun getAllPlayers(): Observable<AllPlayerResponse>
